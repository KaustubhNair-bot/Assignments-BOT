...
class EvaluationMetrics:
    ...
    def answer_completeness(self, query: str, answer: str) -> float:
        ...
        # Length check (medical answers should be detailed)
        if len(answer) > 50:
            score += 0.2
        if len(answer) > 150:
            score += 0.2
        ...
        medical_terms = ['patient', 'diagnosis', 'treatment', 'symptoms', 'condition', 
                        'medical', 'clinical', 'procedure', 'history', 'examination']
        term_count = sum(1 for term in medical_terms if term.lower() in answer.lower())
